...
"""
def minimax(estado, profundidade, jogador, prof_inicial):
    ...
    for cell in celulas_vazias(estado):
        x, y = cell[0], cell[1]
        if (profundidade - prof_inicial) > -5:
            estado[x][y] = jogador
            placar = minimax(estado, profundidade - 1, -jogador, prof_inicial)
            estado[x][y] = 0
...
def IA_vez(comp_escolha, humano_escolha):
    ...
    if profundidade == 16:
        ...
        move = minimax(tabuleiro, profundidade, COMP, prof_inic)
        x, y = move[0], move[1]
...
def HUMANO_vez(comp_escolha, humano_escolha):
    ...
    while (movimento < 1 or movimento > 16):
        try:
            movimento = int(input('Use numero (1..15): '))
